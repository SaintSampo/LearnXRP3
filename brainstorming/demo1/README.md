...
| `compete.html` | Compete C (competition day in four steps) | A module built into the shell rather than framed: Teams (import pairs from the room), Schedule (round-robin, no back-to-back), Play (real 0:30 + 2:00 clock, per-alliance scoring, state buttons, checklist, keyboard shortcuts, projector view), Results (rankings, awards). State persists under `lx.compete`. |
