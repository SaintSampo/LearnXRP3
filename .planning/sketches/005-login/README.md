...
- **F: Two doors, two ways in** — the demo's layout (A plus C) with a role bar above the doors: "Students" over the blue door, "Adults · teachers, mentors, educators" over the white one. No age numbers and no mention of parents, at Jacob's request. The student door keeps code-and-name as the primary path and adds "Sign in with school Google" beneath a divider, with the rules spelled out: school accounts only, under 13 needs the class switch, hashed ID kept, email discarded.
- **G: Who first** — one question before anything else: "I'm a student" or "I'm an adult", as two big buttons. Student shows two ways side by side, code on the board (the usual way) and school Google account. Adult shows the adult sign-in. The answer is a fork, not a record.
